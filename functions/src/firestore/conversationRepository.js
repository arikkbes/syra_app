...
/**
 * Get session state (for deep scan permissions, etc.)
 * @param {string} uid - User ID
 * @param {string} sessionId - Session ID
 */
export async function getSessionState(uid, sessionId) {
  try {
    const sessionRef = db
      .collection("conversation_history")
      .doc(uid)
    ...
    console.error(`[${uid}] Session state load error (session: ${sessionId}):`, e);
    return null;
  }
}

/**
 * Set/update session state
 * @param {string} uid - User ID
 * @param {string} sessionId - Session ID
 * @param {object} partialState - Partial state object to merge
 */
export async function setSessionState(uid, sessionId, partialState) {
  try {
    const sessionRef = db
      .collection("conversation_history")
      .doc(uid)
    ...
    await sessionRef.set(
      {
        sessionState: partialState,
        lastUpdated: FieldValue.serverTimestamp(),
      },
      { merge: true }
    );
    
    console.log(`[${uid}] Session state updated for ${sessionId}:`, partialState);
  } catch (e) {
    console.error(`[${uid}] Session state save error (session: ${sessionId}):`, e);
    throw e;
  }
}